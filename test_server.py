...
def geocode_hk_address(address_text):
    ...
    try:
        ...
        nom_resp = requests.get(nom_url, headers={"User-Agent": "VRP-App"}, params=nom_params, timeout=5)
    ...
    try:
        ...
        als_params = {"q": address_text, "n": 1}
        response = requests.get(als_url, headers=headers, params=als_params, timeout=5)
...
def geocode_batch():
    ...
    for idx, addr in enumerate(addresses):
        coords = geocode_hk_address(addr)
        if coords:
            results.append({
...
def build_matrix():
    ...
    def generate_stream():
        yield json.dumps({"type": "progress", "percent": 5, "message": "Initializing spatial index and KDTree..."}) + "\n"
        ...
        yield json.dumps({"type": "progress", "percent": 15, "message": f"Graph built. Querying {total_edges} routes from OTP..."}) + "\n"
        ...
        for i in range(0, total_edges, chunk_size):
            ...
            yield json.dumps({"type": "progress", "percent": percent, "message": f"Processing OTP Routes: {completed} / {total_edges}"}) + "\n"
            
        formatted_edges = [{"source": res['from_id'], "target": res['to_id'], "time_seconds": res['time_seconds']} for res in otp_matrix_results]
...
def optimize_routes():
    ...
    # REMOVED: time_dimension.SetGlobalSpanCostCoefficient(100)
    # By deleting this, we stop forcing the engine to distribute work equally.
    # It will now prioritize packing jobs tightly to minimize total workers.
    ...
    # This mathematically guarantees the engine finds the absolute minimum fleet size
    ...
    for vehicle_id in range(num_vehicles):
        ...
        while not routing.IsEnd(index):
            ...
            # THE FIX: Call the pure distance_callback directly instead of GetArcCostForVehicle. 
            # This strips the fake 100,000-second fleet penalty out of the final time calculation.
